...
describe("ActionPointResource", () => {
  ...
  test("spends AP when a skill can start", () => {
    ...
    expect(resource.trySpend(2)).toMatchObject({
      accepted: true,
      spentAp: 2,
      missingAp: 0,
      snapshot: { currentAp: 4 },
    });
  });
  ...
  test("rejects a skill without consuming AP when the cost is too high", () => {
    ...
    expect(resource.trySpend(3)).toMatchObject({
      accepted: false,
      spentAp: 0,
      missingAp: 1,
      snapshot: { currentAp: 2 },
    });
  ...
  test("does not regenerate while paused", () => {
    ...
    resource.resume();
    expect(resource.advance(1_000)).toMatchObject({
      currentAp: 4,
      paused: false,
    });
  });
  ...
  test("rejects invalid configuration, costs, and deltas", () => {
    expect(() => new ActionPointResource({ maxAp: -1 })).toThrow(RangeError);
    expect(() =>
      new ActionPointResource({
        regenerationPerSecond: Number.POSITIVE_INFINITY,
      }),
    ).toThrow(RangeError);
