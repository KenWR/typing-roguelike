...
describe("actor effect presentation", () => {
  test("maps known runtime status aliases to provided textures", () => {
    expect(resolveEffectPresentation({
      id: "bleed",
      effectId: "bleeding",
      durationMs: 2_000,
      remainingMs: 1_000,
    }).textureKey).toBe("effect:bleed");
    expect(resolveEffectPresentation({
      id: "weaken",
      effectId: "weakness",
      durationMs: 2_000,
      remainingMs: 1_000,
    }).textureKey).toBe("effect:weaken");
    expect(resolveEffectPresentation({
      id: "delay",
      effectId: "delayed_action",
      durationMs: 2_000,
      remainingMs: 1_000,
    }).textureKey).toBe("effect:enemy-delay");
  });
  ...
  test("maps AP regeneration buff and debuff to their actual icons", () => {
    ...
    expect(effects.map((effect) => effect.textureKey).sort()).toEqual([
      "effect:ap-regen-down",
      "effect:ap-regen-up",
    ]);
  });
  ...
  test("falls back for unknown effects without throwing", () => {
    expect(resolveEffectPresentation({
      ...
      durationMs: null,
      remainingMs: null,
    }).textureKey).toBe(EFFECT_PLACEHOLDER_TEXTURE_KEY);
  });
...
describe("status effect lifetime", () => {
  test("tracks remaining time and removes expired effects while preserving legacy duration", () => {
    ...
    expect(combatant.snapshot.statuses).toEqual([
      { statusId: "bleeding", durationMs: 1_000, stacks: 2 },
    ]);
    expect(combatant.timedStatuses).toEqual([
      { statusId: "bleeding", durationMs: 1_000, remainingMs: 750, stacks: 2 },
    ]);
