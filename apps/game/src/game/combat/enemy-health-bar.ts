...
export class EnemyHealthBar {
  ...
  private refresh(): void {
    ...
    const visibleShieldRatio = Math.min(shieldRatio, Math.max(0, 1 - healthRatio));
    this.hpFill.setSize(ENEMY_HEALTH_BAR_TRACK_WIDTH * healthRatio, TRACK_HEIGHT);
    this.shieldFill
      .setVisible(visibleShieldRatio > 0)
      .setPosition(TRACK_X + ENEMY_HEALTH_BAR_TRACK_WIDTH * healthRatio, TRACK_Y)
      .setSize(ENEMY_HEALTH_BAR_TRACK_WIDTH * visibleShieldRatio, TRACK_HEIGHT);
