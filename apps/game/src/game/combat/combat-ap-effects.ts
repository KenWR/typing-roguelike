...
const parseImmediateApDelta = (skill: SkillDefinition): number => {
  ...
  const leadingRecovery = effect.match(/^AP\s+(\d+(?:\.\d+)?)(?:\s+회복)?(?=$|[,.\s])/);
  if (leadingRecovery !== null) return Number(leadingRecovery[1]);
  const trailingGain = effect.match(/\+\s*AP\s+(\d+(?:\.\d+)?)(?:\D|$)/);
  if (trailingGain !== null) return Number(trailingGain[1]);
  const exactReturn = effect.match(/정확 입력 시 AP\s+(\d+(?:\.\d+)?) 반환/);
  if (exactReturn !== null) return Number(exactReturn[1]);
  return 0;
};

const isSuccessfulRoll = (random: () => number, probability: number): boolean => {
  const value = random();
  if (!Number.isFinite(value) || value < 0 || value >= 1) {
    throw new RangeError("Combat AP effect random value must be in [0, 1).");
  }
  return value < probability;
};

export type CombatEnemyApImpact = Readonly<{
  apDelta?: number;
  defended: boolean;
  perfect: boolean;
}>;
...
export class CombatApEffectController {
  ...
  private memoryFragmentTriggered = false;
  private perfectPeriodTriggers = 0;
  private veteranShieldTriggers = 0;
  private echoCharmTriggers = 0;
  ...
  get resource(): ActionPointResource {
  ...
  resolveSkillCost(skill: SkillDefinition): number {
    ...
    if (this.relicIds.has("relic_memory_fragment") && skill.category === "special" && !this.memoryFragmentTriggered) {
      cost -= 1;
    }

    const isMagic = skill.tags?.some((tag) => tag === "wand" || tag === "staff" || tag === "magic") ?? false;
    if (this.relicIds.has("relic_book_of_wisdom") && isMagic && isSuccessfulRoll(this.random, 0.2)) cost = 0;
  ...
  onSkillStarted(skill: SkillDefinition, comboCount: number): void {
    ...
    if (this.relicIds.has("relic_memory_fragment") && skill.category === "special") this.memoryFragmentTriggered = true;
  ...
  onSkillImpact(skill: SkillDefinition): number {
    ...
    if (this.relicIds.has("relic_hungry_grip") && isMelee && isSuccessfulRoll(this.random, 0.2)) delta += 1;
    // 시간 증가/적 행동 지연 계열 유물은 동일 트리거의 AP 회복으로 통일합니다.
    if (skill.category === "basic") {
      if (this.relicIds.has("relic_stenographer_quill")) delta += 1;
      if (this.relicIds.has("relic_ticklish_gloves") && isSuccessfulRoll(this.random, 0.2)) delta += 1;
      if (this.relicIds.has("relic_delayed_blade") && isSuccessfulRoll(this.random, 0.2)) delta += 1;
    }
    if (this.relicIds.has("relic_echo_charm") && this.echoCharmTriggers < 2 && isSuccessfulRoll(this.random, 0.15)) {
      this.echoCharmTriggers += 1;
      delta += 1;
    }
  ...
  onEnemyImpact({ apDelta = 0, defended, perfect }: CombatEnemyApImpact): number {
    let delta = apDelta;
    if (defended && this.relicIds.has("relic_veteran_shield") && this.veteranShieldTriggers < 2) {
      this.veteranShieldTriggers += 1;
      delta += 1;
    }
    if (perfect && this.relicIds.has("relic_perfect_period") && this.perfectPeriodTriggers < 3) {
      this.perfectPeriodTriggers += 1;
      delta += 1;
    }
    if (perfect && this.relicIds.has("relic_red_hourglass")) delta += 1;
