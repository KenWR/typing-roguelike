...
export class InventoryModal {
  ...
  constructor(
    ...
    this.blocker = scene.add
      .rectangle(width / 2, height / 2, width, height, 0x020611, 0.72)
      .setInteractive();
    this.blocker.on(
    ...
    this.panel = scene.add
      .rectangle(0, 0, 1, 1, 0x0b1220, 0.98)
      .setOrigin(0)
    ...
    this.scrollTrack = scene.add
      .rectangle(0, 0, 4, 1, 0x1e293b, 1)
      .setOrigin(0);
    this.scrollThumb = scene.add
      .rectangle(0, 0, 4, 1, 0x5eead4, 1)
      .setOrigin(0);
  ...
  layout(width: number, height: number): void {
    ...
    this.blocker
      .setPosition(safeWidth / 2, safeHeight / 2)
    ...
    this.titleIcon
      .setPosition(
        layout.panelX + layout.padding + titleIconSize / 2,
        layout.panelY + (layout.compact ? 24 : 29),
      )
      .setScale(titleIconSize / 28);
    this.title
      .setPosition(
        layout.panelX + layout.padding + titleIconSize + 8,
        layout.panelY + (layout.compact ? 14 : 14),
      )
      .setFontSize(layout.compact ? "20px" : "26px");
    this.subtitle
      .setPosition(
        layout.panelX + layout.padding + titleIconSize + 8,
        layout.panelY + (layout.compact ? 38 : 45),
      )
      .setText(`장비 ${this.view.equipment.length}개 · 유물 ${this.view.relics.length}개`);
    this.closeButton
      .setPosition(
        layout.panelX + layout.panelWidth - layout.padding,
        layout.panelY + (layout.compact ? 24 : 30),
      )
    ...
    const equipmentWidth = wide
      ? Math.floor((layout.contentWidth - columnGap) * 0.58)
      : layout.contentWidth;
    const relicWidth = wide
      ? layout.contentWidth - equipmentWidth - columnGap
      : layout.contentWidth;
    ...
    this.scrollOffset = clampInventoryModalScroll(
      this.scrollOffset,
      this.maxScroll,
    );
    this.applyScroll();
  ...
  private applyScroll(): void {
    ...
    this.scrollTrack
      .setPosition(this.scrollbarX, this.contentY)
      .setSize(4, this.contentHeight)
    ...
    const thumbHeight = Math.max(
      28,
      this.contentHeight * (this.contentHeight / (this.contentHeight + this.maxScroll)),
    );
    const scrollRatio = this.maxScroll === 0
      ? 0
      : -this.scrollOffset / this.maxScroll;
    this.scrollThumb
      .setPosition(
        this.scrollbarX,
        this.contentY + (this.contentHeight - thumbHeight) * scrollRatio,
      )
  ...
  private renderEquipmentSection(
    ...
    const count = this.addText(
      this.contentRoot,
      x + width,
      y + 3,
      `${this.view.equipment.length}개`,
      {
        color: "#fcd34d",
    ...
    for (const equipment of this.view.equipment) {
      cursorY += this.renderEquipmentCard(
        x,
        cursorY,
        width,
        equipment,
  ...
  private renderEquipmentCard(
    ...
    const name = this.addText(card, textX, padding - 1, equipment.name, {
      color: "#f8fafc",
    ...
    const accent = this.scene.add
      .rectangle(0, 0, 4, cardHeight, RARITY_COLORS[equipment.rarity], 0.92)
      .setOrigin(0);
  ...
  private renderRelicCard(
    ...
    const name = this.addText(card, textX, padding - 1, relic.name, {
      color: "#f8fafc",
      fontSize: compact ? "12px" : "15px",
      fontStyle: "bold",
    }, textWidth);
    ...
    const accent = this.scene.add
      .rectangle(0, 0, 4, cardHeight, RARITY_COLORS[relic.rarity], 0.92)
      .setOrigin(0);
    card.addAt(background, 0);
    card.addAt(accent, 1);
    this.contentRoot.add(card);
  ...
  private getEquipmentIconTextureKey(
    equipment: InventoryEquipmentView,
  ): string | undefined {
    const textureKey = resolveEquipmentIconTextureKey(equipment.id);
    return textureKey !== undefined && this.scene.textures.exists(textureKey)
      ? textureKey
      : undefined;
  }
