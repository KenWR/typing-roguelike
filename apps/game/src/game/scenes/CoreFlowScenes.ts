...
export class MapScene extends EmptyCoreScene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, 42, `MAP · ${view.floor}F`, { fontFamily, fontSize: "36px", color: "#f9fafb" }).setOrigin(0.5);
    this.add.text(width / 2, 78, syncStatus.message, { fontFamily, fontSize: "14px", color: syncStatus.mode === "local_fallback" ? "#fbbf24" : "#9ca3af" }).setOrigin(0.5);
    ...
    this.add.text(leftHudX + 22, 184, `골드  ${view.currencyText}`, { fontFamily, fontSize: "20px", color: "#f9fafb" });
    this.add.text(leftHudX + 22, 218, `장비  ${view.equipmentText}`, { fontFamily, fontSize: "18px", color: "#d1d5db", wordWrap: { width: hudPanelWidth - 50 } });
    this.add.text(width / 2, 118, `현재 위치 · ${view.currentLocation}`, { fontFamily, fontSize: "22px", color: "#f9fafb", align: "center", wordWrap: { width: Math.max(260, width - leftHudX - hudPanelWidth - 24) } }).setOrigin(0.5);
    this.add.text(width / 2, 172, `경로  ${view.pathText}`, { fontFamily, fontSize: "18px", color: "#9ca3af", align: "center", wordWrap: { width: Math.max(260, width - leftHudX - hudPanelWidth - 24) } }).setOrigin(0.5);
