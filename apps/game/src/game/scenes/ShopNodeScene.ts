...
import { createShopModalInputGuardLayout, stopShopModalPointerPropagation } from "../shop/shop-modal-input-guard";
...
export class ShopNodeScene extends Phaser.Scene {
  ...
  private toggleEquipmentModal(): void {
    this.hideOfferTooltip();
    if (this.equipmentModal !== undefined) {
    ...
    const inputBlocker = this.add
      .rectangle(blockerLayout.x, blockerLayout.y, blockerLayout.width, blockerLayout.height, 0x000000, 0.001)
      .setInteractive();
    inputBlocker.on(
    ...
    panel.add(
      this.add
        .rectangle(width / 2, height / 2, Math.min(width - 80, 760), Math.min(height - 80, 600), 0x111827, 0.98)
        .setStrokeStyle(2, 0x6b7280),
    );
    panel.add(
      this.add
        .text(width / 2, 80, "현재 장비", { fontFamily: "Galmuri9, monospace", fontSize: "28px", color: "#f9fafb" })
        .setOrigin(0.5),
    );
    ...
    const text =
      equipped.length === 0
        ? "장착한 장비가 없습니다."
        : equipped.map(formatEquipmentInfo).join("\n\n────────────\n\n");
    panel.add(
      this.add.text(width / 2 - 330, 125, text, {
        fontFamily: "Galmuri9, monospace",
        fontSize: "15px",
        color: "#e5e7eb",
        wordWrap: { width: 660 },
  ...
  private handlePurchase(offer: ShopOffer, replacementRingId?: string | null): void {
    this.hideOfferTooltip();
    const equippedRingIds = [this.flow.runState.loadout.ring1Id, this.flow.runState.loadout.ring2Id].filter(
      (id): id is string => id !== null,
    );
    if (
      replacementRingId === undefined &&
      offer.kind === "ring" &&
      equippedRingIds.length === 2 &&
      !this.flow.purchasedOfferIds.has(offer.id) &&
      !ownsShopOffer(this.flow.runState, offer) &&
      this.flow.runState.runCurrency >= offer.price
    ) {
      this.showRingReplacementModal(offer, equippedRingIds);
      return;
    }
    const beforeCurrency = this.flow.runState.runCurrency;
    this.flow = purchaseShopOffer(this.flow, offer.id, replacementRingId);
    const purchased = this.flow.purchasedOfferIds.has(offer.id) && this.flow.runState.runCurrency < beforeCurrency;
    if (purchased) {
      playCoinSound();
      this.feedbackText?.setText(offer.kind === "relic" ? "유물을 장착했습니다." : "구매 완료").setColor("#86efac");
      this.syncSession();
      this.syncCheckpoint();
    } else if (this.flow.purchasedOfferIds.has(offer.id))
      this.feedbackText?.setText("이미 구매한 상품입니다.").setColor("#fbbf24");
    else if (ownsShopOffer(this.flow.runState, offer))
      this.feedbackText
        ?.setText(offer.kind === "relic" ? "이미 보유한 유물입니다." : "이미 보유한 장비입니다.")
        .setColor("#fbbf24");
    else this.feedbackText?.setText("골드가 부족합니다.").setColor("#fca5a5");
    this.refresh();
  }

  private showRingReplacementModal(offer: ShopOffer, equippedRingIds: readonly string[]): void {
    ...
    modal.add(
      this.add.rectangle(width / 2, height / 2, panelWidth, panelHeight, 0x111827, 1).setStrokeStyle(2, 0xfcd34d),
    );
    modal.add(
      this.add
        .text(width / 2, height / 2 - 100, "반지 교체", {
          fontFamily: "Galmuri9, monospace",
          fontSize: "26px",
          color: "#f9fafb",
        })
    ...
    modal.add(
      this.add
        .text(width / 2, height / 2 - 58, "장착 중인 반지 하나를 버리고 구매하시겠습니까?", {
          fontFamily: "Galmuri9, monospace",
          fontSize: "15px",
          color: "#cbd5e1",
        })
    ...
    equippedRingIds.forEach((id, index) => {
      const name = RING_BY_ID.get(id)?.name ?? id;
      this.addRingReplacementButton(modal, `${name} 버리기`, id, width / 2, height / 2 - 12 + index * 48, offer);
    });
    this.addRingReplacementButton(modal, "새 반지 버리기", null, width / 2, height / 2 + 84, offer);
  ...
  private addRingReplacementButton(
    ...
    const button = this.add
      .text(x, y, label, {
        ...
        fontSize: "17px",
        color: "#fff7ed",
        backgroundColor: replacementRingId === null ? "#334155" : "#7f1d1d",
        padding: { x: 16, y: 8 },
      })
      .setOrigin(0.5)
      .setInteractive({ useHandCursor: true });
    button.on("pointerdown", () => {
      ...
      this.handlePurchase(offer, replacementRingId);
    });
    modal.add(button);
  }
  ...
  private showOfferTooltip(offer: ShopOffer, anchor: Phaser.Geom.Rectangle): void {
    ...
    if (details.textureKey !== undefined && this.textures.exists(details.textureKey)) {
      tooltip.add(
        this.add
          .rectangle(TOOLTIP_PADDING, TOOLTIP_PADDING, TOOLTIP_IMAGE_SIZE, TOOLTIP_IMAGE_SIZE, 0x1e293b, 1)
          .setOrigin(0)
      ...
      const scale = Math.min((TOOLTIP_IMAGE_SIZE - 12) / image.width, (TOOLTIP_IMAGE_SIZE - 12) / image.height, 1.5);
      image.setScale(scale);
      tooltip.add(image);
    } else {
      tooltip.add(
        this.add
          .rectangle(TOOLTIP_PADDING, TOOLTIP_PADDING, TOOLTIP_IMAGE_SIZE, TOOLTIP_IMAGE_SIZE, 0x1f2937, 1)
          .setOrigin(0)
          .setStrokeStyle(1, 0x475569),
      );
      tooltip.add(
        this.add
          .text(TOOLTIP_PADDING + TOOLTIP_IMAGE_SIZE / 2, TOOLTIP_PADDING + TOOLTIP_IMAGE_SIZE / 2, "이미지\n없음", {
            fontFamily: "Galmuri9, monospace",
            fontSize: "14px",
            color: "#94a3b8",
            align: "center",
    ...
    const meta = this.add.text(
      textX,
      name.y + name.height + 8,
      `${details.kindLabel} · ${RARITY_LABELS[details.rarity] ?? details.rarity}`,
      { fontFamily: "Galmuri9, monospace", fontSize: "14px", color: "#f5cf72" },
    );
    tooltip.add(meta);
    const description = this.add.text(textX, meta.y + meta.height + 10, details.description, {
      fontFamily: "Galmuri9, monospace",
      fontSize: "14px",
      lineSpacing: 3,
      color: "#e2e8f0",
    ...
    const price = this.add.text(TOOLTIP_PADDING, TOOLTIP_PADDING + TOOLTIP_IMAGE_SIZE + 12, `${offer.price}G`, {
      fontFamily: "Galmuri9, monospace",
      fontSize: "16px",
      color: "#fde68a",
    });
  ...
  private refresh(): void {
    ...
    this.flow.offers.forEach((offer, index) => {
      ...
      const owned = ownsShopOffer(this.flow.runState, offer);
      const status = purchased ? "구매 완료" : owned ? "보유 중" : "";
      const button = this.add
        .text(
          36,
          SHOP_OFFER_TOP + index * SHOP_OFFER_ROW_HEIGHT,
        ...
        .setInteractive({ useHandCursor: !purchased && !owned });
  ...
  private syncSession(): void {
    ...
    if (active?.status === "active") runSession.update(() => this.flow.runState);
  }
