...
export class ShopNodeScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.rerollButton = this.add.text(36, 310, "", { fontFamily: "Galmuri9, monospace", fontSize: "18px", color: "#f9fafb", backgroundColor: "#374151", padding: { x: 14, y: 9 } }).setInteractive({ useHandCursor: true });
    this.rerollButton.on("pointerdown", () => this.handleReroll());
    this.add.text(220, 310, "상점 나가기", { fontFamily: "Galmuri9, monospace", fontSize: "18px", color: "#f9fafb", backgroundColor: "#1f6f68", padding: { x: 14, y: 9 } }).setInteractive({ useHandCursor: true }).once("pointerdown", () => {
  ...
  private handlePurchase(offer: ShopOffer): void {
    const beforeCurrency = this.flow.runState.runCurrency; const beforeOwned = this.flow.runState.inventory.itemInstances.length;
    this.flow = purchaseShopOffer(this.flow, offer.id);
    const purchased = this.flow.runState.runCurrency < beforeCurrency && this.flow.runState.inventory.itemInstances.length > beforeOwned;
    if (purchased) { playCoinSound(); this.feedbackText?.setText("구매 완료").setColor("#86efac"); this.syncSession(); this.syncCheckpoint(); }
    else if (this.flow.purchasedOfferIds.has(offer.id)) this.feedbackText?.setText("이미 구매한 상품입니다.").setColor("#fbbf24");
    else if (this.flow.runState.inventory.itemInstances.includes(offer.equipmentId)) this.feedbackText?.setText("이미 보유한 장비입니다.").setColor("#fbbf24");
  ...
  private refresh(): void {
    ...
    this.flow.offers.forEach((offer, index) => {
      const purchased = this.flow.purchasedOfferIds.has(offer.id); const owned = this.flow.runState.inventory.itemInstances.includes(offer.equipmentId); const status = purchased ? "구매 완료" : owned ? "보유 중" : "";
      const button = this.add.text(36, 142 + index * 52, `${formatShopOfferLabel(offer)}${status ? ` · ${status}` : ""}`, { fontFamily: "Galmuri9, monospace", fontSize: "18px", color: purchased || owned ? "#9ca3af" : "#f9fafb", backgroundColor: purchased || owned ? "#1f2937" : "#243247", padding: { x: 12, y: 8 } });
      if (!purchased && !owned) { button.setInteractive({ useHandCursor: true }); button.on("pointerdown", () => this.handlePurchase(offer)); }
