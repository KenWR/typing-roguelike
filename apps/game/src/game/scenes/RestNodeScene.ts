...
export class RestNodeScene extends Phaser.Scene {
  ...
  init(data: RestNodeSceneData): void {
    this.flow = createRestNodeFlow(data.runState, data.nodeId, data.nextNodeIds);
  ...
  create(): void {
    ...
    this.add.text(36, 82, `HP ${this.flow.runState.character.currentHp} / ${this.flow.runState.character.maxHp}`, {
      fontFamily: "Galmuri9, monospace",
      fontSize: "18px",
      color: "#d1fae5",
    });
    ...
    this.createAction(36, 192, "그냥 쉬고 돌아가기", () => this.finish());
  }

  private createAction(x: number, y: number, label: string, action: () => void): void {
    this.add.text(x, y, label, {
      ...
      padding: { x: 14, y: 9 },
    }).setInteractive({ useHandCursor: true }).once("pointerdown", action);
  }
