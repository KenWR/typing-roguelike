...
import {
  ...
  ENEMY_DEFAULT_SPECIAL_WINDUP_OFFSET_MS,
  ENEMY_DEFENSE_WINDUP_MS,
} from "./enemy-windup-values.ts";
...
interface EnemyDefinition {
  id: keyof typeof ENEMY_ATTACK_WINDUP_MS;
  name: string;
...
const createAttackAction = (enemy: EnemyDefinition): EnemyActionConfig => ({
  ...
  windupMs: ENEMY_ATTACK_WINDUP_MS[enemy.id],
  recoveryMs: 300,
...
const createDefenseAction = (enemy: EnemyDefinition): EnemyActionConfig => ({
  ...
  name: "방어",
  damage: 0,
  windupMs: ENEMY_DEFENSE_WINDUP_MS,
  recoveryMs: 500,
...
const createSpecialActions = (enemy: EnemyDefinition): readonly EnemyActionConfig[] => {
  const specials = enemy.specials ?? [{
    ...
    windupMs: ENEMY_ATTACK_WINDUP_MS[enemy.id] + ENEMY_DEFAULT_SPECIAL_WINDUP_OFFSET_MS,
    recoveryMs: 500,
