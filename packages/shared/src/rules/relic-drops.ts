...
const isDropRarity = (rarity: Rarity): rarity is RelicDropRarity => rarity !== "hidden";
...
export const generateRelicRewardCandidates = ({
  ...
  const available = RELIC_CONFIGS.filter((relic) => isDropRarity(relic.rarity) && !excludedIds.has(relic.id));
  ...
  while (candidates.length < count && available.length > 0) {
    const eligibleEntries = table.filter(
      ({ rarity, weight }) => weight > 0 && available.some((relic) => relic.rarity === rarity),
    );
    ...
    const selectedRelic = available[selectedIndex];
    if (selectedRelic === undefined) break;
    candidates.push(selectedRelic);
...
export const ownsRelic = (runState: Readonly<RunState>, relicId: string): boolean =>
  runState.inventory.relicInstances.includes(relicId);
 ...
 */
export const applyRelicAcquisition = (runState: Readonly<RunState>, relicId: string): RunState => {
  const relic = RELIC_CONFIGS.find((candidate) => candidate.id === relicId);
  if (relic === undefined) throw new RangeError(`Unknown relic: ${relicId}`);
  if (ownsRelic(runState, relicId)) return runState as RunState;

  const maxHpIncrease = relic.effects
    .map((effect) => effect.match(/^maxHp \+(\d+)$/))
    .find((match): match is RegExpMatchArray => match !== null);
  const maxHpCap = relic.effects
    .map((effect) => effect.match(/^maxHp cap (\d+)$/))
    .find((match): match is RegExpMatchArray => match !== null);
  const heal = relic.effects
    .map((effect) => effect.match(/^heal (\d+)$/))
    .find((match): match is RegExpMatchArray => match !== null);
  const increasedMaxHp = runState.character.maxHp + (maxHpIncrease === undefined ? 0 : Number(maxHpIncrease[1]));
  const cappedMaxHp = Math.max(
    runState.character.maxHp,
    Math.min(increasedMaxHp, maxHpCap === undefined ? increasedMaxHp : Number(maxHpCap[1])),
  );
  const healedCurrentHp = Math.min(
    cappedMaxHp,
    runState.character.currentHp + (heal === undefined ? 0 : Number(heal[1])),
  );

  return {
    ...runState,
    character:
      cappedMaxHp === runState.character.maxHp && healedCurrentHp === runState.character.currentHp
        ? runState.character
        : { currentHp: healedCurrentHp, maxHp: cappedMaxHp },
    inventory: {
      ...runState.inventory,
