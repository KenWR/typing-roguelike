...
export const RELIC_DAMAGE_MODIFIERS: ReadonlyMap<string, readonly RelicDamageModifier[]> = new Map([
  ["relic_red_ink", [{ priority: "final", multiplier: 0.1 }]],
  ["relic_whetstone", [{ priority: "category", multiplier: 0.2, skillCategories: ["basic"] }]],
  ...
  [
    relicId("도박사의 주사위"),
    [
      { priority: "category", multiplier: 0.15, skillCategories: ["basic"] },
      { priority: "category", multiplier: -0.15, skillCategories: ["special"] },
    ],
  ],
...
export const DAMAGE_RELIC_IDS_BY_CATEGORY: Readonly<Record<SkillCategory, readonly string[]>> = Object.freeze({
  basic: RELIC_CONFIGS.filter((relic) => relic.skillTags.includes("basic") && RELIC_DAMAGE_MODIFIERS.has(relic.id)).map(
    (relic) => relic.id,
  ),
  special: RELIC_CONFIGS.filter(
    (relic) => relic.skillTags.includes("special") && RELIC_DAMAGE_MODIFIERS.has(relic.id),
  ).map((relic) => relic.id),
  guard: RELIC_CONFIGS.filter((relic) => relic.skillTags.includes("guard") && RELIC_DAMAGE_MODIFIERS.has(relic.id)).map(
    (relic) => relic.id,
  ),
});
...
const isConditionMet = (condition: RelicDamageModifier["condition"], input: DamageCalculationInput): boolean => {
  switch (condition) {
    case undefined:
      return true;
    case "targetHpHalf":
      return (input.targetHpRatio ?? 1) <= 0.5;
    case "enemyCountThree":
      return (input.enemyCount ?? 0) >= 3;
    case "fastInput":
      return (input.inputDurationMs ?? Infinity) <= 500;
    case "targetGuarding":
      return input.targetGuarding === true;
    case "enemyWindingUp":
      return input.enemyWindingUp === true;
    case "bothHpLow":
      return (input.targetHpRatio ?? 1) <= 0.3 && (input.playerHpRatio ?? 1) <= 0.3;
    case "twoHandedWeapon":
      return input.hasTwoHandedWeapon === true;
    case "recoveryLong":
      return (input.recoveryMs ?? 0) >= 600;
    case "repeatedCharacter":
      return input.hasRepeatedCharacter === true;
    case "floor":
      return (input.floor ?? 0) > 0;
  }
};
...
export const calculateRelicDamage = (input: DamageCalculationInput): DamageCalculationResult => {
  ...
  for (const relicId of DAMAGE_RELIC_IDS_BY_CATEGORY[input.skillCategory]) {
    ...
    for (const modifier of RELIC_DAMAGE_MODIFIERS.get(relicId) ?? []) {
      ...
      bonuses[modifier.priority] +=
        modifier.condition === "floor" ? modifier.multiplier * (input.floor ?? 0) : modifier.multiplier;
  ...
  const damage = Math.max(
    1,
    Math.round(baseDamage * (1 + bonuses.category) * (1 + bonuses.conditional) * (1 + bonuses.final)),
  );
  return {
    damage,
    baseDamage,
    categoryBonus: bonuses.category,
    conditionalBonus: bonuses.conditional,
    finalBonus: bonuses.final,
    appliedRelicIds,
  };
};
