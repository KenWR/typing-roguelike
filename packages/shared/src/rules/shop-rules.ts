...
import { applyRelicAcquisition, generateRelicRewardCandidates, getRelicPrice, ownsRelic } from "./relic-drops.ts";
import { applyRingAcquisition, generateRingRewardCandidates, getRingPrice, ownsRing } from "./ring-drops.ts";
...
export const normalizeShopOffer = (offer: ShopOffer | LegacyShopOffer): ShopOffer =>
  "kind" in offer ? offer : { id: offer.id, kind: "equipment", itemId: offer.equipmentId, price: offer.price };
...
export type ShopPurchaseResult = Readonly<{
  applied: boolean;
  reason: "purchased" | "already_purchased" | "already_owned" | "insufficient_currency";
  offer: ShopOffer;
...
export const createShopOffers = ({
  ...
  const available = [
    ...new Map(equipment.filter(({ rarity }) => rarity !== "hidden").map((item) => [item.id, item] as const)).values(),
  ];
...
export const findShopOfferEquipment = (offer: ShopOffer): EquipmentConfig | undefined =>
  offer.kind === "equipment" ? EQUIPMENT_CONFIGS.find((candidate) => candidate.id === offer.itemId) : undefined;

export const findShopOfferRelic = (offer: ShopOffer): RelicConfig | undefined =>
  offer.kind === "relic" ? RELIC_CONFIGS.find((candidate) => candidate.id === offer.itemId) : undefined;

export const findShopOfferRing = (offer: ShopOffer): RingConfig | undefined =>
  offer.kind === "ring" ? RING_CONFIGS.find((candidate) => candidate.id === offer.itemId) : undefined;

/** 진열된 상품을 이미 보유하고 있는지 판단합니다. */
export const ownsShopOffer = (runState: Readonly<RunState>, offer: ShopOffer): boolean => {
...
export const applyShopPurchase = ({
  ...
  if (offer.kind === "ring") {
    const acquired = applyRingAcquisition(runState, offer.itemId, { replaceRingId: replacementRingId });
    return {
      applied: true,
      reason: "purchased",
      offer,
  ...
  const equipment = EQUIPMENT_CONFIGS.find((candidate) => candidate.id === offer.itemId);
  const acquired =
    equipment === undefined
      ? {
          ...runState,
