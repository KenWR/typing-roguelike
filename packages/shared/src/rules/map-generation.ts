...
const DIAGONAL_EDGES: readonly (readonly [MapNodeChoice, MapNodeChoice])[] = [[1, 2], [2, 1], [2, 3], [3, 2]];
...
const shuffle = <T>(values: readonly T[], seed: number): T[] => {
	const result = [...values];
	for (let index = result.length - 1; index > 0; index -= 1) { const swapIndex = hash(`${seed}:${index}`) % (index + 1); [result[index], result[swapIndex]] = [result[swapIndex], result[index]]; }
	return result;
};
...
const validateChoicePath = (round: number, choicePath: readonly number[]): void => {
	if (choicePath.length !== round - 1) throw new RangeError(`A round ${round} map choice path must contain ${round - 1} choices.`);
	for (const choice of choicePath) if (!Number.isInteger(choice) || choice < 1 || choice > MAX_MAP_CHOICES) throw new RangeError("Map choices must be integers from 1 to 3.");
};
...
export const getMapNodeKey = (round: number, path: readonly number[]): string => {
	validateRound(round);
	if (path.length !== round) throw new RangeError(`A round ${round} map node path must contain ${round} choices.`);
	for (const choice of path) if (!Number.isInteger(choice) || choice < 1 || choice > MAX_MAP_CHOICES) throw new RangeError("Map choices must be integers from 1 to 3.");
...
const nextChoicesFor = (seed: number, round: number, choice: MapNodeChoice): MapNodeChoice[] => {
...
const generateRoundNodes = (seed: number, round: number, monsterIds: readonly string[] = []): GeneratedMapNode[] => {
	const types = getNodeTypes(seed, round);
	return types.map((type, index) => {
		const choice = NODE_CHOICES[index]!;
		const key = nodeKey(round, choice);
		const parentKey = round === 1 ? START_NODE_KEY : nodeKey(round - 1, choice);
		const nextNodeKeys = nextChoicesFor(seed, round, choice).map((nextChoice) => nodeKey(round + 1, nextChoice));
		const node: GeneratedMapNode = { choice, icon: type, iconType: type, key, parentKey, nextNodeKeys, round, type };
		if ((type === "combat" || type === "elite" || type === "boss") && monsterIds.length > 0) node.monsterId = monsterIds[hash(`${seed}:${round}:${choice}`) % monsterIds.length];
		return node;
	});
};
...
export const generateMap = (seed: number, monsterIds: readonly string[] = []): GeneratedMap => {
	validateSeed(seed);
	return { seed, rounds: Array.from({ length: MAP_ROUND_COUNT }, (_, index) => { const round = index + 1; return { round, nodes: generateRoundNodes(seed, round, monsterIds) }; }) };
};
