...
describe("relic drop table", () => {
  ...
  test("honours weight overrides", () => {
    ...
    expect(requireValue(table.find((entry) => entry.rarity === "legendary")).probability).toBeGreaterThan(0.5);
  });
...
describe("relic reward candidates", () => {
  ...
  test("rejects a random source outside [0, 1)", () => {
    expect(() => generateRelicRewardCandidates({ count: 1, random: () => 1 })).toThrow(RangeError);
  });
});

describe("relic acquisition", () => {
  test("stores the relic and equips it so combat effects apply", () => {
    const relic = requireValue(RELIC_CONFIGS[0]);
  ...
  test("applies immediate max HP and healing effects when acquiring dried herb", () => {
    const initial = {
      ...createInitialRunState({ seed: 12, maxHp: 150, initialHp: 140 }),
      character: { currentHp: 140, maxHp: 150 },
    };
    const runState = applyRelicAcquisition(initial, "relic_dried_herb");

    expect(runState.character).toEqual({ currentHp: 155, maxHp: 160 });
  });
  ...
  test("keeps previously acquired relics", () => {
    ...
    const firstRelic = requireValue(first);
    const secondRelic = requireValue(second);
    let runState = applyRelicAcquisition(createInitialRunState({ seed: 9 }), firstRelic.id);
    runState = applyRelicAcquisition(runState, secondRelic.id);

    expect(runState.inventory.relicInstances).toEqual([firstRelic.id, secondRelic.id]);
    expect(runState.build.equippedRelicIds).toEqual([firstRelic.id, secondRelic.id]);
  });

  test("does not mutate the input run state", () => {
    const initial = createInitialRunState({ seed: 10 });
    applyRelicAcquisition(initial, requireValue(RELIC_CONFIGS[0]).id);
  ...
  test("rejects an unknown relic id", () => {
    expect(() => applyRelicAcquisition(createInitialRunState({ seed: 11 }), "relic_nope")).toThrow(RangeError);
  });
});
