...
describe("map node state", () => {
	...
	test("clears the selected node and exposes only connected next nodes", () => {
		...
		expect(result.map.nodeStatuses["1-2"]).toBe("locked");
		expect(result.map.nodeStatuses["1-3"]).toBe("locked");
